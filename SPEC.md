...
Apply the appropriate direction profile to the real inserter.
Let Factorio determine whether the inserter can pick up and drop an item.
Observe whether the transfer proceeds.
Restore or switch profiles according to the two-way state machine.
...
When a player opens a two-way inserter GUI, normalize the inserter to the forward profile.
Treat the vanilla GUI as the forward-profile editor.
Suspend reverse probing / reverse switching while the relevant inserter GUI is open.
Save forward changes as needed.
Resume normal two-way operation after the GUI is closed.
...
Add two-way mode to existing inserter entities.
Preserve the placed direction as the permanent primary direction.
Implement safe forward/reverse pickup/drop switching.
Implement reverse probing after successful forward transfer.
Implement periodic reverse probing while forward is idle.
Add persistent forward/reverse logical profiles.
Add independent reverse filters.
Add independent reverse stack size override.
Add the reverse configuration GUI.
Make GUI-open behavior normalize/suspend correctly.
Add Smart Inserters shared-position compatibility through public interfaces only.
Evaluate red-wire-forward / green-wire-reverse circuit behavior.
Add blueprint/copy/upgrade persistence.
Tune probe timing and UPS behavior.
## 29. Open questions

The following items are intentionally unresolved and require implementation testing or further design discussion:

What is the ideal reverse-probe interval?
10 ticks is an initial discussion value only.
What exact Factorio inserter state/event should be used to identify the safest direction-switch boundary?
Which normal inserter properties beyond filters and stack size should be direction-specific in the first release?
Spoilage priority is a candidate.
Full circuit-control parity is not yet confirmed.
Can the vanilla inserter control behavior cleanly use:
...
What is the best relative-GUI placement when Smart Inserters or other inserter GUI mods are installed?
How should reverse profile data be encoded into blueprints and entity settings copy/paste?
Which modded inserters, if any, require explicit compatibility exclusions?
## 30. Core design rule
